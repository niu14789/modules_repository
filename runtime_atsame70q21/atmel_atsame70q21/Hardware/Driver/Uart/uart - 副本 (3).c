...
//	  FS_SHELL_INIT(u_s_art_callback_7s,u_s_art_callback,0,_CB_IT_|_CB_PRIORITY_(0)|_CB_IT_TPYE_(0)|_CB_IT_IRQN_(USART2_IRQn)|_CB_ENABLE_);
		FS_SHELL_INIT(dma,dma,0,_CB_IT_|_CB_PRIORITY_(0)|_CB_IT_TPYE_(0)|_CB_IT_IRQN_(XDMAC_IRQn)|_CB_DISABLE_);
...
int u_s_art_config(void * p_arg , int argc)
{
	...
	/* u(s)art config rx */
		...
		if(ucm->rx_mode == 0x0001)
		{
			...
			{			
				uart_index = ucm->index - 5;//usart0~2
				USART_EnableRecvTimeOut(usart_dma[uart_index].pUsartHw, 100);  // 104us/bit * 10bit
...
struct file * u_s_art_open(FAR struct file * filp)
{
   ...
   length = strlen(	filp->f_path);
	 ...
	 if(length == 0x7)
	 {
		 ...
		 if(index < 8)
		 {
			  if(filp_s[index].f_oflags != __FS_IS_INODE_OK)
				{
...
unsigned int u_s_art_read(FAR struct file *filp, FAR char *buffer, unsigned int buflen)
{
		...
		char index = 0;
		unsigned int len = 0;
		unsigned char *u_s_pbuff;
		unsigned int buffsize;
	  static unsigned int cda_last[5];
		/* 66666666666666 */
		if(filp->f_multi > 7)
		...
		{ 
			 index = filp->f_multi - 5;
			 u_s_pbuff = chs_txrx[index][1].pBuff;
			 /* is LLI mode ? */
			 if(uart_config[filp->f_multi] & 0x20)
			 {
				  buffsize = chs_txrx[index][1].BuffSize;
			 }else
			 {
		...
		{
			 index = filp->f_multi;
			 u_s_pbuff = ch_txrx[index][1].pBuff;
			
			 if( uart_dma[index].pXdmad->pXdmacs->XDMAC_CHID[uart_dma[index].pRxChannel->ChNum].XDMAC_CDA != cda_last[index] )
			 {
				 cda_last[index] = uart_dma[index].pXdmad->pXdmacs->XDMAC_CHID[uart_dma[index].pRxChannel->ChNum].XDMAC_CDA;
				 buffsize = ch_txrx[index][1].BuffSize;//* 2 ;
			 }else
			 {
				 buffsize = 0;
			 }
			 /* dma rx callback clear the cache */
		...
		if( buffsize )
		{
			if(filp->f_multi > 4)
			{
				 len = ( buffsize > buflen ) ? buflen : buffsize;

				 /* data copy */
				 memcpy(buffer , u_s_pbuff , len);				
			}
			else
			{
				 if(read_offset[filp->f_multi] < 0)
					 read_offset[filp->f_multi] = 0;
				 else
				 {/* use DMA deepth 64, DMA copy doesn't immediately, will miss 4 byte in 64 - 128(get 64 - 124) 
					  MAX DMA deepth is 60*/
					 len = ( buffsize > buflen ) ? buflen : buffsize;

					 /* data copy */
					 if(len <= buffsize*2 - read_offset[filp->f_multi])
					 {
						 memcpy(buffer , u_s_pbuff+read_offset[filp->f_multi] , len);				 
					 }
					 else
					 {
						 memcpy(buffer , u_s_pbuff+read_offset[filp->f_multi] , buffsize - read_offset[filp->f_multi]);
						 memcpy(buffer , u_s_pbuff , len - (buffsize - read_offset[filp->f_multi]));					 
					 }
					 read_offset[filp->f_multi] = (read_offset[filp->f_multi] + len) % (buffsize*2);
				 }
			}
			 /* disable the data */
			 uart_config[filp->f_multi] &=~ 0x10;
			 /* return */
			 return len;
		}else
		{
