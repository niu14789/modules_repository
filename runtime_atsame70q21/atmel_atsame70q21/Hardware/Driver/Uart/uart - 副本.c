		...
		if( buffsize )
		{
			...
			else
			{
				 ...
				 {/* use DMA deepth 64, DMA copy doesn't immediately, will miss 4 byte in 64 - 128(get 64 - 124) 
					 ...
					 /* data copy */
					 if(len <= buffsize*2 - read_offset)
					 {
						 memcpy(buffer , u_s_pbuff+read_offset , len);				 
					 }
					 else
					 {
						 memcpy(buffer , u_s_pbuff+read_offset , buffsize - read_offset);
						 memcpy(buffer , u_s_pbuff , len - (buffsize - read_offset));					 
					 }
					 read_offset = (read_offset + len) % (buffsize*2);
				 }
			}
			 /* disable the data */
